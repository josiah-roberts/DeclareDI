...
using static DeclareDI.ServiceRegistrationTarget;
...
namespace DeclareDI
{
  public static class ServiceCollectionExtensions
  {
    ...
    public static void AddByAttributes(this IServiceCollection services) => services.AddByAttributes(Assembly.GetCallingAssembly());
    public static void AddByAttributes(this IServiceCollection services, Assembly assembly)
    {
      IEnumerable<(Type, ServiceAttribute, Type)> ToTypeRegistrations((Type Type, ServiceAttribute ServiceAttribute) serviceInfo) =>
        serviceInfo.ServiceAttribute.Target switch
        {
          ...
          _ =>
            throw new InvalidOperationException($"Unsupported {nameof(ServiceRegistrationTarget)} {serviceInfo.ServiceAttribute.Target}")
        };

      IEnumerable<(MethodInfo, ServiceAttribute, Type)> ToMethodRegistrations((MethodInfo Method, ServiceAttribute ServiceAttribute) serviceInfo) =>
        serviceInfo.ServiceAttribute.Target switch
        {
          ImplementedInterfaces => serviceInfo.Method.ReturnType.GetInterfaces() switch
          {
            ...
            _ =>
              throw new InvalidOperationException($"Method {serviceInfo.Method.Name} cannot be registered as {nameof(ImplementedInterfaces)}, because {serviceInfo.Method.ReturnType} does not implement any interfaces.")
          },
      ...
      foreach (var (method, ServiceAttribute, registerAsType) in serviceFactoryMethods)
        services.Add(new ServiceDescriptor(registerAsType, AsServiceFactory(method), ServiceAttribute.Lifetime));
